...
func main() {
	...
	router.HandleFunc("identity/{namespace}/{identifier}/requests/history/{count}", FetchRequestHistory).Methods("GET")

	// IDP
	// https://app.swaggerhub.com/apis/NDID/identity_provider/1.0
	router.HandleFunc("/identity/{namespace}/{identifier}", RetrieveCallbackUrl).Methods("GET")
	...
	router.HandleFunc("/rp/requests/reference/{reference_number}", GetAsServiceInfo).Methods("GET")
